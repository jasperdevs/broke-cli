...
export function createReadFileTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ path, offset, limit, mode, tail, refresh }) => readFileMaybeRemote({ path, cwd, offset, limit, mode, tail, refresh }),
  });
}

export function createWriteFileTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ path, content }) => writeFileDirect({ path, cwd, content }),
  });
}

export function createEditFileTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ path, old_string, new_string }) => editFileDirect({ path, cwd, old_string, new_string }),
  });
}

export function createListFilesTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ path, maxDepth, include }) => listFilesMaybeRemote({ path, cwd, maxDepth, include }),
  });
}

export function createGrepTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ pattern, path, include }) => grepDirect({ pattern, path, cwd, include }),
  });
}

export function createSemSearchTool(cwd = process.cwd()) {
  return tool({
    ...
    execute: async ({ query, path, include, limit }) => semSearchDirect({ query, path, cwd, include, limit }),
  });
}
